...
# Post-installation script for BAIclaw on Linux
...
# Create symbolic link for the BAIclaw app binary
if [ -x /opt/BAIclaw/baiclaw ]; then
    ln -sf /opt/BAIclaw/baiclaw /usr/local/bin/baiclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/BAIclaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/BAIclaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/BAIclaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/BAIclaw/resources/apparmor-profile'
...
echo "BAIclaw has been installed successfully."
