...
### Web3 / AgentWallet
在 **设置 → Web3** 中通过四步 **创建 AgentWallet** 向导导入 **TRON** 私钥、设置本地**主密码**（`@bankofai/agent-wallet` 加密存储），并在平台返回绑定地址时与 BANK OF AI API Key 账户校验一致；成功页展示示例提问。可复制地址、删除钱包。钱包数据默认在 **`~/.agent-wallet`**，可通过环境变量 **`AGENT_WALLET_DIR`** 覆盖（与 CLI 一致）；新建钱包的 id 为 **`default_secure`**，与 `agent-wallet start local_secure -w default_secure` 对齐。旧版本曾使用 Electron **userData** 下的 `agent-wallet` 目录，如需沿用请将其内容复制到 `~/.agent-wallet`。若仅用 CLI 初始化保管库、本应用尚未写入 `kv-password` 文件：应用也会按 CLI 习惯读取 **`runtime_secrets.json`** 或环境变量 **`AGENT_WALLET_PASSWORD`**；也可在 **设置 → Web3** 输入一次主密码（`POST` `/api/agent-wallets/unlock`，随后写入系统密钥链）。默认目录为 Electron 的 **`app.getPath('home')`** + **`.agent-wallet`**（文件夹名为两个字母 l，勿写成 `.agent-walllet`）。若启动应用时设置了 **`AGENT_WALLET_DIR`**，命令行需指向同一路径（`agent-wallet list -d "<路径>"`）。**设置 → Web3** 会显示 **`GET` `/api/agent-wallets`** 返回的 **`storagePath`**。删除**最后一个**已配置钱包时，会删除**整个保管库目录**（与 `storagePath` 一致，例如 `~/.agent-wallet`），含 `kv-password` 与 `master.json`。Host API：`GET` `/api/agent-wallets`（含 `storagePath`、`vaultUnlockRequired` 等）、`POST` `/api/agent-wallets/unlock`、`POST` `/api/agent-wallets/validate-private-key`、`POST` `/api/agent-wallets`、`DELETE` `/api/agent-wallets/:id`。
